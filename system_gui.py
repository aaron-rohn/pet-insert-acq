...
class SystemUI():
    ...
    def callback_gen(self, func, *args):
        return lambda: self.print([self.to_mask(getattr(b, func)(*args)) for b in self.backend])

    def power_callback_gen(self, update):
        def cb():
            ret = []
            for b in self.backend:
                nxt_state = [v.get() for v in b.m_pow_var]
                r = b.get_set_frontend_power(update, nxt_state)
                ret.append(self.to_mask(r))
            self.print(ret)
        return cb
    ...
    def __init__(self, system_instance):
        ...
        rx_status_callback = self.callback_gen('get_rx_status')
        tx_status_callback = self.callback_gen('get_tx_status')
        power_rd_callback  = self.power_callback_gen(False)
        power_wr_callback  = self.power_callback_gen(True)
        current_callback   = lambda: self.print([b.get_current() for b in self.backend])

        self.backend_refresh   = tk.Button(self.root, text = "Refresh", command = self.refresh_callback)
